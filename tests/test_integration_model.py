...
pytestmark = pytest.mark.skipif(
    __import__("os").environ.get("RUN_MODEL_INTEGRATION") != "1",
    reason="set RUN_MODEL_INTEGRATION=1 to run model/golden integration tests",
)
...
def run_server_in_thread(port: int, default_model: str = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"):
    # Keep golden integration tests deterministic even when the host has a GPU.
    # Production defaults remain device-aware (CUDA->fp16; capable CPU->bf16, else fp32).
    cfg_content = (
        f"port={port}\n"
        "log_level=INFO\n"
...
def grpc_channel():
    # Golden files were generated with CPU/FP32 inference.  Force that runtime
    # explicitly even if this test is launched from a CUDA-capable environment.
    previous_runtime_target = os.environ.get("GRAPHWISE_RUNTIME_TARGET")
    previous_config = os.environ.get("GRAPHWISE_CONFIG")
    os.environ["GRAPHWISE_RUNTIME_TARGET"] = "cpu"
    ...
    try:
        ...
        try:
            ...
            pass
        if previous_runtime_target is None:
            os.environ.pop("GRAPHWISE_RUNTIME_TARGET", None)
        else:
            os.environ["GRAPHWISE_RUNTIME_TARGET"] = previous_runtime_target
        if previous_config is None:
            os.environ.pop("GRAPHWISE_CONFIG", None)
        else:
            os.environ["GRAPHWISE_CONFIG"] = previous_config
